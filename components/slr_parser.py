...
class SLRParser:
    ...
    def print_info(self):
        ...
        return (self.C, self.parse_table, self.parse_table_symbols) 
    ...
    def generate_automaton(self):
        ...
        # automaton.view('automata.jpg')
        automaton.render(directory='static', filename='automata.dot')
    ...
    def print_LR_parser(self, results):
        ...
        # header = [history[0] for history, max_len in zip(results.values(), max_lens.values())]
        temp = list(results.values())
        body = [[temp[j][i] for j in range(len(temp))] for i in range(len(temp[0]))]
